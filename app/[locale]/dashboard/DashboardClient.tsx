...
export default function DashboardClient({ serverStats }: DashboardClientProps) {
  ...
  const STORAGE_LIMIT_BYTES = 250 * 1024 * 1024 // 250MB
  const storagePercentage = Math.min((serverStats.totalSize / STORAGE_LIMIT_BYTES) * 100, 100)
      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-surface border border-white/5 rounded-2xl p-6 hover:border-white/10 transition-colors">
            <p className="text-xs text-muted font-body mb-2">Total Files</p>
            <h3 className="text-2xl font-display font-700 text-paper">{serverStats.totalFiles}</h3>
          </div>

          {/* Card 2: Total Downloads */}
          <div className="bg-surface border border-white/5 rounded-2xl p-6 hover:border-white/10 transition-colors">
            <p className="text-xs text-muted font-body mb-2">Total Downloads</p>
            <h3 className="text-2xl font-display font-700 text-paper">{serverStats.totalDownloads}</h3>
          </div>

          {/* Card 3: Storage Used */}
          <div className="bg-surface border border-white/5 rounded-2xl p-6 hover:border-white/10 transition-colors">
            <p className="text-xs text-muted font-body mb-2">Storage Used</p>
            <div className="flex items-baseline gap-2 mb-2">
              <h3 className="text-2xl font-display font-700 text-paper">{formatBytes(serverStats.totalSize)}</h3>
              <span className="text-xs text-muted">/ 250MB</span>
            </div>
            {/* Progress bar */}
